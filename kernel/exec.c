...
int exec(char *path, char **argv) {
  ...
  printf("DEBUG: exec completed for pid=%d, name=%s\n", p->pid, p->name);
  if(p->pid == 1) {
    printf("DEBUG: Calling vmprint for init process\n");
    vmprint(p->pagetable);
    printf("DEBUG: vmprint completed\n");
  } else {
    printf("DEBUG: Not calling vmprint, pid=%d (not 1)\n", p->pid);
  }
